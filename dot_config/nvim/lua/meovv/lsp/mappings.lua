...
-- toggle inlay hints
map("n", "<leader>lh", "<cmd>lua vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled())<cr>",
  { desc = "Toggle inlay hints for buffer" })

-- code actions
map("n", "<leader>gn", "<cmd>lua vim.lsp.buf.rename()<cr>", { desc = "Rename" })
map("n", "<leader>la", "<cmd>lua vim.lsp.buf.code_actions()<cr>", { desc = "Code Actions" })
